...
typedef struct
{
  ...
  uint16_t         Vbus;
} Observer_Inputs_t;//观测器输入


int16_t SPD_GetElAngle(const SpeednPosFdbk_Handle_t *pHandle);//获取电角度

int32_t SPD_GetMecAngle(const SpeednPosFdbk_Handle_t *pHandle);//获取机械角度

int16_t SPD_GetAvrgMecSpeedUnit(const SpeednPosFdbk_Handle_t *pHandle);//平均机械速度

int16_t SPD_GetElSpeedDpp(const SpeednPosFdbk_Handle_t *pHandle);//平均电速度
